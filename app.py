...
class Supplier(db.Model):
    ...
    delivery_days = db.Column(db.String(120))  # e.g., "Mon,Wed,Fri"
    lead_time_days = db.Column(db.Integer, default=1)

class Item(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(200), nullable=False)
    unit = db.Column(db.String(50), default="個")  # 個, 本, g, mL, kg, L
    location = db.Column(db.String(50), default="常温")  # 常温/冷蔵/冷凍/消耗
    par = db.Column(db.Integer, default=0)  # 標準在庫
    reorder_point = db.Column(db.Integer, default=0)  # 発注点
...
def ensure_bootstrap_user():
    # 初回用に admin / admin123 を自動作成（本番は変更して下さい）
    if User.query.count() == 0:
        u = User(username="admin", role="admin")
        u.set_password("admin123")
        db.session.add(u)
        db.session.commit()
        print("✅ 初期ユーザーを作成しました： username=admin / password=admin123  ※ログイン後に必ず変更してください")

def current_stock_map():
    # 種別ごとに符号をつけて集計（adj は +/− どちらでもOK）
    qty_signed = func.sum(
        case(
            ...
            else_=StockMovement.qty,  # adj
        )
    )
    rows = db.session.query(StockMovement.item_id, func.coalesce(qty_signed, 0.0)).group_by(StockMovement.item_id).all()
...
def inventory():
    ...
    # suggested_order = max(0, par - current_stock)
    data = []
    for it in items:
        now = stock.get(it.id, 0.0)
        suggested = max(0, int((it.par or 0) - now))
        data.append((it, now, suggested))
...
def add_movement():
    if request.method == "POST":
        ...
        m = StockMovement(item_id=item_id, kind=kind, qty=qty, note=note, user_id=current_user.id)
        db.session.add(m)
        db.session.commit()
